package dataStructure.datastructure.BTree;
...
public class BTree<E> implements Iterable<E> {
        ...
        while (true) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                for (int j = 0; j < order; j++) {
                    ...
                    p[i].values[j] = nodes[index].values[size - 1];
                    nodes[index].values[size - 1] = null;
                    p[i].size++;
                    nodes[index].size--;
                    nodes[index].parent = p[i];
                    p[i].children[j] = nodes[index];
                }
            }
   ...
   public boolean contains(E key,Node<E> p){
        if (p==null){
            return false;
        }
        if (p.isLeaf){
   ...
   public boolean containsAll(Collection<? extends E>c){
    ...
    public void dumpToList(Node<E> p, List<E> values) {
        if (p == null) {
            return;
        }
        if (p.isLeaf) {
